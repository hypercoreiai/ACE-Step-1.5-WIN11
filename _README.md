...
If you installed **nano-vllm** manually, use **`uv run --no-sync acestep`** instead, so uv does not uninstall it (and a dependency) on start.

Open http://localhost:7860 in your browser. Models will be downloaded automatically on first run.

**Why does `uv run` uninstall packages (e.g. nano-vllm)?**  
`uv run` syncs the environment with the lock file before starting. That can **uninstall** your manually installed `nano-vllm` (and a dependency, e.g. flash-attn) and **not reinstall** it if the path dependency cannot be resolved from your current directory. To keep your manual install and avoid that, start with:
