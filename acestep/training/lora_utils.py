...
def _patch_peft_torchao_dispatch() -> None:
    """Patch peft's dispatch_torchao to pass get_apply_tensor_subclass (required by TorchaoLoraLinear).

    PEFT's dispatch_torchao does not pass get_apply_tensor_subclass when creating TorchaoLoraLinear
    for int8-quantized (AffineQuantizedTensor) models, causing TypeError. This patches the module
    to inject the missing argument. ACE-Step uses Int8WeightOnlyConfig.
    """
    try:
        ...
        return
    if not hasattr(peft_torchao, "dispatch_torchao") or not hasattr(peft_torchao, "TorchaoLoraLinear"):
        return

    import inspect
    sig = inspect.signature(peft_torchao.TorchaoLoraLinear.__init__)
    if "get_apply_tensor_subclass" not in sig.parameters:
        return  # Newer peft may have made it optional

    _orig_dispatch = peft_torchao.dispatch_torchao

    def _patched_dispatch(target, adapter_name, lora_config=None, config=None, **kwargs):
        cfg = lora_config or config
        # Inject get_apply_tensor_subclass for int8 quantized layers (ACE-Step uses Int8WeightOnlyConfig)
        if "get_apply_tensor_subclass" not in kwargs:
            try:
                from torchao.quantization import Int8WeightOnlyConfig
                kwargs["get_apply_tensor_subclass"] = lambda: Int8WeightOnlyConfig()
            except ImportError:
                pass
        return _orig_dispatch(target, adapter_name, lora_config=cfg, config=cfg, **kwargs)

    # Patch both modules: peft.tuners.lora.model keeps its own reference from "from .torchao import dispatch_torchao",
    # so we must patch it there too; otherwise the dispatchers list uses the pre-import reference.
    peft_torchao.dispatch_torchao = _patched_dispatch
    if hasattr(peft_lora_model, "dispatch_torchao"):
        peft_lora_model.dispatch_torchao = _patched_dispatch
...
def inject_lora_into_dit(
    ...
    # Workaround for peft+torchao: TorchaoLoraLinear requires get_apply_tensor_subclass but
    # dispatch_torchao does not pass it (peft bug). Patch so we inject it for int8 quantized models.
