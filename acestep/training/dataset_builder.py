...
class DatasetBuilder:
    ...
    def preprocess_to_tensors(
        ...
        # device may be str (e.g. "cuda") or torch.device
        is_cuda = (getattr(device, "type", device) == "cuda") if isinstance(device, torch.device) else str(device).startswith("cuda")
        logger.info(f"[Preprocess] device={device}, vae on {vae_device}, {len(labeled_samples)} labeled samples")
        if vae_device.type == "cpu":
            logger.warning("[Preprocess] VAE is on CPU: encoding will be very slow. Consider initializing without CPU offload for preprocessing.")
        ...
        for i, sample in enumerate(labeled_samples):
            ...
            try:
                ...
                logger.info(f"[Preprocess] Sample {i+1}/{len(labeled_samples)}: {sample.filename} (max_duration={max_duration}s)")
                ...
                # Log size: long clips make VAE encode very slow (especially on CPU)
                duration_s = audio.shape[1] / sr if sr else 0
                logger.info(f"[Preprocess]   Loaded audio: {audio.shape[1]} samples ({duration_s:.1f}s @ {sr}Hz)")
                ...
                # Use VAE's device so input and model weights match (avoids CUDA/CPU dtype error)
                audio = audio.unsqueeze(0).to(vae_device).to(vae.dtype)
                _t1 = time.perf_counter()
                logger.info(f"[Preprocess]   Step 1 (load+resample): {_t1 - t0:.1f}s")

                # Step 2: VAE encode audio to get target_latents
                t_vae = time.perf_counter()
                with torch.no_grad():
                    latent = vae.encode(audio).latent_dist.sample()
                    # [1, 64, T_latent] -> [1, T_latent, 64]; move result to training device/dtype
                    target_latents = latent.transpose(1, 2).to(device).to(dtype)
                if is_cuda:
                    torch.cuda.synchronize()
                logger.info(f"[Preprocess]   Step 2 (VAE encode): {time.perf_counter() - t_vae:.1f}s")
                ...
                if is_cuda:
                    torch.cuda.synchronize()
                logger.info(f"[Preprocess]   Step 4 (text encode): {time.perf_counter() - t_text0:.1f}s")
                ...
                if is_cuda:
                    torch.cuda.synchronize()
                logger.info(f"[Preprocess]   Step 7 (condition encoder): {time.perf_counter() - t_enc:.1f}s")
                ...
                success_count += 1
                logger.info(f"[Preprocess]   Saved {os.path.abspath(output_path)} (sample total: {time.perf_counter() - sample_start:.1f}s)")
