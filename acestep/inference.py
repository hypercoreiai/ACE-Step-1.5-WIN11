...
def generate_music(
    ...
    try:
        ...
        if use_lm:
            ...
            # set cot caption and language if needed (never overwrite user-provided lyrics)
            ...
            # Use LM-generated lyrics for DiT only when user did not provide lyrics
            if not (params.lyrics or "").strip() and lm_generated_metadata:
                lm_lyrics = lm_generated_metadata.get("lyrics") or ""
                if lm_lyrics:
                    dit_input_lyrics = lm_lyrics

        # Phase 2: DiT music generation (dit_input_caption / dit_input_lyrics = user input or LM when empty)
