...
class LLMHandler:
    ...
    def _initialize_5hz_lm_vllm(self, model_path: str) -> str:
        ...
        try:
            ...
            start_time = time.time()
            # Force distributed init to use localhost so c10d does not try to connect to hostname (e.g. android-i74dot2:2333).
            os.environ["MASTER_ADDR"] = "127.0.0.1"
            os.environ.setdefault("MASTER_PORT", "29500")
            # Guard against "trying to initialize the default process group twice" when
            # Gradio triggers Initialize twice or nano-vllm is the PyPI build (no is_initialized check).
            import torch.distributed as dist
            _init_orig = dist.init_process_group
            def _init_once(*a, **k):
                if dist.is_initialized():
                    return
                return _init_orig(*a, **k)
            dist.init_process_group = _init_once
            try:
                self.llm = LLM(
                    model=model_path,
                    enforce_eager=False,
                ...
                dist.init_process_group = _init_orig
