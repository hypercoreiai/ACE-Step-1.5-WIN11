...
def handle_format_sample(
    ...
    Returns a single tuple of 9 values (Gradio does not iterate generators for this event).
    """
    if not llm_handler.llm_initialized:
        gr.Warning(t("messages.lm_not_initialized"))
        return (
    ...
    if key_scale and str(key_scale).strip():
        user_metadata['keyscale'] = str(key_scale).strip()
    if time_signature and str(time_signature).strip():
        user_metadata['timesignature'] = str(time_signature).strip()
    ...
    if not result.success:
        ...
        return (
            ...
            gr.update(),
            gr.update(),
            str(result.status_message or t("messages.format_failed")),
        )
    ...
    # Coerce to safe types so Gradio never receives invalid values
    out_caption = result.caption if result.caption is not None else ""
    out_lyrics = result.lyrics if result.lyrics is not None else ""
    out_bpm = result.bpm
    out_keyscale = result.keyscale if result.keyscale is not None else ""
    out_language = result.language if result.language is not None else ""
    out_timesignature = result.timesignature if result.timesignature is not None else ""
    out_status = result.status_message if result.status_message is not None else t("messages.format_success")
