...
def _subtitle_path_if_exists(path):
    """Return path only if the file exists; otherwise None. Prevents Gradio ValueError when VTT was deleted."""
...
def cleanup_on_stop(batch_queue):
    """On Stop: delete all temp files referenced in batch_queue and clear queue to free memory.
    Returns state updates to clear the results UI.
    """
    if not batch_queue:
        return (
            {},
            ...
            None,
            t("messages.stopped_cleared"),
            "Batch 0 / 0",
        )
    deleted_count = 0
    dirs_to_remove = set()
    for batch_index, data in list(batch_queue.items()):
            ...
            if p.lower().endswith(".json"):
                try:
                    if os.path.isfile(p):
                        os.remove(p)
                        deleted_count += 1
                    dirs_to_remove.add(os.path.dirname(p))
                except OSError as e:
                    logger.warning(f"[cleanup_on_stop] Could not remove {p}: {e}")
            else:
                try:
                    if os.path.isfile(p):
                        os.remove(p)
    ...
    for d in dirs_to_remove:
        try:
            ...
            pass
    logger.info(f"[cleanup_on_stop] Deleted {deleted_count} temp file(s), cleared batch queue (freed tensor memory)")
    return (
        {},
        0,
        0,
...
def generate_with_progress(
    ...
    """Generate audio with progress tracking. Caption and lyrics must come from the click inputs only (current UI values)."""
    # Use only the passed-in caption/lyrics from the click event (never from state or batch)
    ...
    logger.info(f"[generate_with_progress] Using caption/lyrics from UI: caption={repr(captions[:80])}{'...' if len(captions) > 80 else ''}, lyrics={repr(lyrics[:80])}{'...' if len(lyrics) > 80 else ''}")
    ...
    # Yield a quick status update so the client connection stays alive during the long save/score/lrc phase
    n_audios = len(audios)
    _skip8 = (gr.skip(), gr.skip(), gr.skip(), gr.skip(), gr.skip(), gr.skip(), gr.skip(), gr.skip())
    yield (
        *_skip8,
        gr.skip(), gr.skip(),
        f"Preparing audio outputs (1/{n_audios})...",
        gr.skip(),
        *_skip8, *_skip8, *_skip8, *_skip8,
        gr.skip(), gr.skip(),
        None, None,
    )
    ...
    for i in range(8):
        if i < len(audios):
            # Yield progress so the frontend doesn't time out during save/score/lrc
            yield (
                *_skip8,
                gr.skip(), gr.skip(),
                f"Saving audio {i + 1}/{n_audios}...",
                gr.skip(),
                *_skip8, *_skip8, *_skip8, *_skip8,
                gr.skip(), gr.skip(),
                None, None,
            ...
            # ============== STEP 1: Yield progress + CLEAR LRC (do NOT update audio here) ==============
            # Only the final yield sets audio paths. Updating audio in the loop can cause the player to
            # "hang" on an old song when the frontend applies updates out of order or caches the first.
            current_audio_updates = [gr.skip() for _ in range(8)]
            # current_audio_updates[i] = audio_path  # intentionally omitted
    ...
    for idx in range(min(8, len(audio_outputs))):
        if audio_outputs[idx] is not None:
            final_codes_display_updates[idx] = gr.update(value=final_codes_list[idx], visible=True)
    final_accordion_updates = [gr.skip() for _ in range(8)]
    
    # Brief pause so the client can apply previous progress updates before the final state
    time_module.sleep(0.08)
    # Final yield: send explicit audio paths for every slot so UI always gets complete state
    # (avoids FE hang when switching batch sizes - last update must have all values)
    # Only pass subtitle path if file exists (VTT may have been deleted manually or by cleanup)
    # Use explicit value=None for empty slots (not gr.skip()) so the client never keeps stale audio.
    final_audio_updates = []
    for idx in range(8):
        path = audio_outputs[idx] if idx < len(audio_outputs) else None
        sub = final_subtitles_list[idx] if idx < len(final_subtitles_list) else None
        sub = _subtitle_path_if_exists(sub)
        if path is not None:
            final_audio_updates.append(gr.update(value=path, subtitles=sub, playback_position=0))
        else:
            final_audio_updates.append(gr.update(value=None, subtitles=None))
    yield (
...
def update_audio_subtitles_from_lrc(lrc_text: str, audio_duration: float = None):
    ...
    # Only pass path if file exists (avoid ValueError if temp was deleted)
    return gr.update(subtitles=_subtitle_path_if_exists(vtt_path))


def sync_audio_from_batch_queue(batch_queue, current_batch_index):
    """Apply the current batch's audio paths to the 8 audio components.
    Run as a .then() after generation so the UI reliably shows the latest result.
    If the batch is empty or missing (e.g. .then() ran before state was updated),
    return gr.skip() for all slots so we never overwrite the player with None (blank).
    Short delay so we read state after the generator's output is committed (avoids alternating blank).
    """
    time_module.sleep(0.12)
    if not batch_queue or not isinstance(batch_queue, dict):
        return (gr.skip(),) * 8
    batch = batch_queue.get(current_batch_index, {})
    audio_paths = batch.get("audio_paths", [])
    real_paths = [p for p in audio_paths if p and not str(p).lower().endswith(".json")]
    if not real_paths:
        return (gr.skip(),) * 8
    subtitles_list = batch.get("subtitles", [None] * 8)
    if not isinstance(subtitles_list, (list, tuple)):
        subtitles_list = [None] * 8
    out = []
    for idx in range(8):
        path = real_paths[idx] if idx < len(real_paths) else None
        sub = subtitles_list[idx] if idx < len(subtitles_list) else None
        sub = _subtitle_path_if_exists(sub)
        if path is not None:
            out.append(gr.update(value=path, subtitles=sub, playback_position=0))
        else:
            out.append(gr.skip())
    return tuple(out)
...
def generate_with_batch_management(
    dit_handler, llm_handler,
    captions, lyrics, simple_query_input, bpm, key_scale, time_signature, vocal_language,
    inference_steps, guidance_scale, random_seed_checkbox, seed,
    reference_audio, audio_duration, batch_size_input, src_audio,
    text2music_audio_code_string, repainting_start, repainting_end,
    instruction_display_gen, audio_cover_strength, task_type,
    ...
    Caption and lyrics must always come from the click inputs (current UI), never from generation_params_state.
    """
    # Use only the clicked caption/lyrics (never from state) so changing description then generating uses new text
    captions = captions if captions is not None else ""
    lyrics = lyrics if lyrics is not None else ""
    if not isinstance(captions, str):
        captions = str(captions) if captions else ""
    if not isinstance(lyrics, str):
        lyrics = str(lyrics) if lyrics else ""
    # Simple mode: "song description" (simple_query_input) used when caption is empty.
    # When both are non-empty, prefer simple_query so the description field wins (avoids stale
    # captions from queue/state on 3rd+ generate when FE sends old values).
    try:
        simple_query = str(simple_query_input).strip() if simple_query_input else ""
    except Exception:
        simple_query = ""
    if simple_query:
        # Prefer simple_query when caption empty, or when both set (description field = source of truth in simple mode)
        captions = simple_query
        logger.info("[generate_with_batch_management] Using simple_query_input as caption")

    # Require at least one of: caption, lyrics, or (in simple mode) song description.
    # Avoids running the full pipeline with no user prompt (random generation, GPU/CPU load, and possible FE hang).
    if not (captions or lyrics) or (not captions.strip() and not lyrics.strip()):
        logger.info("[generate_with_batch_management] Skipping generation: no song description, caption, or lyrics provided")
        _skip8 = (gr.skip(),) * 8
        yield (
            *_skip8,  # generated_audio_1..8
            gr.skip(),  # generated_audio_batch
            gr.skip(),  # generation_info
            t("messages.no_description_no_generate"),  # status_output
            gr.skip(),  # seed
            *_skip8, *_skip8, *_skip8, *_skip8,  # scores, codes, accordions, lrc_display
            gr.skip(), gr.skip(), gr.skip(), gr.skip(), gr.skip(),  # lm_metadata, is_format, batch_index, total_batches, batch_queue
            gr.skip(), gr.skip(), gr.skip(), gr.skip(), gr.skip(),  # generation_params_state, batch_indicator, prev_btn, next_btn, next_status
            gr.skip(),  # restore_params_btn
            gr.update(interactive=True),  # generate_btn - re-enable so user can try again
        )
        return

    logger.info(f"[generate_with_batch_management] batch_size_input={batch_size_input} caption_len={len(captions)} lyrics_len={len(lyrics)} caption_preview={repr(captions[:60])}{'...' if len(captions) > 60 else ''}")
    ...
    # 44: lm_metadata, 45: is_format_caption, 46: extra_outputs, 47: raw_codes_list
    #
    # Pass through the actual audio updates (0-7) so the UI receives the definitive state.
    # (A previous "clear then set" yielded two updates; the second was not applied reliably and left the player empty.)
    ui_result = result[:-2] if len(result) > 47 else (result[:-1] if len(result) > 46 else result)
...
def generate_next_batch_background(
    ...
    captions = (params.get("captions") or "").strip()
    lyrics = (params.get("lyrics") or "").strip()
    if not captions and not lyrics:
        logger.info("[generate_next_batch_background] Skipping: no caption/lyrics in saved params")
        return (
            batch_queue,
            total_batches,
            "",
