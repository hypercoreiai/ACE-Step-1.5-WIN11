...
"""
    ...
    # ========== Generation Handler ==========
    # Generate click event (stored so Stop can cancel it)
    gen_click = generation_section["generate_btn"].click(
    ...
    # Stop button: cancel running generation and clear memory + temp files
    generation_section["stop_btn"].click(
        fn=res_h.cleanup_on_stop,
        inputs=[results_section["batch_queue"]],
        outputs=[
            results_section["batch_queue"],
            results_section["current_batch_index"],
            results_section["total_batches"],
    ...
    # Sync audio components from batch_queue so the player and Save always show the latest result
    # (the generator's final yield is sometimes not applied by the frontend on subsequent runs)
    gen_click.then(
        fn=res_h.sync_audio_from_batch_queue,
        inputs=[
            results_section["batch_queue"],
            results_section["current_batch_index"],
        ],
