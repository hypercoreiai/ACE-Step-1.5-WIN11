...
class AceStepHandler:
    ...
    def _decode_audio_codes_to_latents(self, code_str: str) -> Optional[torch.Tensor]:
        ...
        Clamps LM-generated indices to the quantizer's valid range to avoid CUDA index-out-of-bounds.
        """
        ...
        with self._load_model_context("model"):
            ...
            # Infer codebook size so we can clamp LM indices (LM may emit IDs outside valid range)
            codebook_max = None
            if hasattr(quantizer, "codebooks") and quantizer.codebooks is not None:
                cb = quantizer.codebooks
                if hasattr(cb, "shape"):
                    # ResidualFSQ: codebooks shape often (num_quantizers, codebook_size, dim)
                    if len(cb.shape) >= 2:
                        codebook_max = cb.shape[1] - 1
                elif isinstance(cb, (list, tuple)) and len(cb) > 0 and hasattr(cb[0], "shape"):
                    codebook_max = cb[0].shape[0] - 1
            if codebook_max is None:
                codebook_max = getattr(quantizer, "codebook_size", 64000) - 1
            codebook_max = max(0, int(codebook_max))
            # Clamp parsed indices to valid range to avoid CUDA assert (LM can emit out-of-range IDs)
            clamped = [max(0, min(int(i), codebook_max)) for i in code_ids]
            n_clamped = sum(1 for a, b in zip(code_ids, clamped) if a != b)
            if n_clamped:
                logger.warning(f"[_decode_audio_codes_to_latents] Clamped {n_clamped} audio code indices to [0, {codebook_max}] (quantizer codebook range)")
            code_ids = clamped
